...
public class HelloKVLite {
	...
	public static void main(String[] args) {
		...
		// This initializes an arbitrary key-value pair and stores it in KVLite.
		// The key-value structure is /helloKey : $value
		String secondKeyString = "ethanKey", secondValueString = "CS342 is the greatest!";
		Key secondKey = Key.createKey(Arrays.asList(secondKeyString));
		Value secondValue = Value.createValue(secondValueString.getBytes());
		store.put(secondKey, secondValue);

		// R(ead)
		// This queries KVLite using the same key.
		// The result, a byte array, is converted into a string.
		String secondResult = new String(store.get(secondKey).getValue().getValue());
		System.out.println(secondKey.toString() + " : " + secondResult);

		// U(pdate)
		// This modifies the value, puts the new value, overwriting the old
		// value, and gets/prints it again.
		secondValueString = "CS232 is the best class!";
		secondValue = Value.createValue(secondValueString.getBytes());
		store.put(secondKey, secondValue);
		secondResult = new String(store.get(secondKey).getValue().getValue());
		System.out.println(secondKey.toString() + " : " + secondResult);

		// D(elete)
		// This deletes the key-value pair.
		store.delete(secondKey);
		if (store.get(secondKey) == null) {
			System.out.println("Good, key successfully deleted...");
		} else {
			System.out.println("Oops, key not deleted...");
		}

		/****************** Second key-value pair complete ***********/
